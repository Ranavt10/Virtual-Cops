...
public class CampaignModeController : BaseModeController
{
    ...
    public override void InitMode()
    {
        ...
        //if (GameDataNEW.staticStageRewardData.ContainsKey(GameDataNEW.currentStage.id))
        //{
        //    StaticStageRewardData rewardData = GameDataNEW.staticStageRewardData[GameDataNEW.currentStage.id];
        //    expWin = rewardData.expWin[(int)GameDataNEW.currentStage.difficulty];
        //}
    ...
    public override void StartGame()
    {
        ...
        if (GameDataNEW.playerTutorials.IsCompletedStep(TutorialType.ActionInGame) == false)
        {
            if (string.Compare(GameDataNEW.currentStage.id, "1.1") == 0)
            {
    ...
    public override void EndGame(bool isWin)
    {
        ...
        if (isWin)
        {
            ...
            if (MapUtils.IsStagePassed(GameDataNEW.currentStage.id, GameDataNEW.currentStage.difficulty) == false)
            {
                rewards = GameDataNEW.staticCampaignStageData.GetFirstTimeRewards(GameDataNEW.currentStage.id, GameDataNEW.currentStage.difficulty);
            ...
            MapUtils.UnlockCampaignProgress(GameDataNEW.currentStage);
    ...
    private void SetProgressDailyQuest()
    {
        ...
        if (GameDataNEW.selectingBoosters.Count > 0)
            EventDispatcher.Instance.PostEvent(EventID.UseBooster);

        if (GameDataNEW.selectingBoosters.Contains(BoosterType.Damage))
            EventDispatcher.Instance.PostEvent(EventID.UseBoosterDamage);

        if (GameDataNEW.selectingBoosters.Contains(BoosterType.Critical))
            EventDispatcher.Instance.PostEvent(EventID.UseBoosterCritical);

        if (GameDataNEW.selectingBoosters.Contains(BoosterType.Speed))
            EventDispatcher.Instance.PostEvent(EventID.UseBoosterSpeed);

        if (GameDataNEW.selectingBoosters.Contains(BoosterType.CoinMagnet))
    ...
    private void SpawnEnemyFromSide(Vector2 position)
    {
        if (GameDataNEW.mode == GameMode.Campaign)
        {
            ...
            int maxLevel = GameDataNEW.staticCampaignStageData.GetLevelEnemy(GameDataNEW.currentStage.id, GameDataNEW.currentStage.difficulty);
    ...
    {
        Map mapPrefab = MapUtils.GetMapPrefab(GameDataNEW.currentStage.id);
