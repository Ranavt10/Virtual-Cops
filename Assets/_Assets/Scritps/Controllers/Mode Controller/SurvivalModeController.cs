...
public class SurvivalModeController : BaseModeController
{
    ...
    public override void EndGame(bool isWin)
    {
        ...
        if (totalScore > 0)
        {
            GameDataNEW.playerTournamentData.score += totalScore;
           // TournamentData tournamentData = new TournamentData(AccessToken.CurrentAccessToken.UserId, GameDataNEW.playerTournamentData.score, GetMostUsedGunId(), false);
    ...
    private void OnQuitSurvivalSession()
    {
        ...
        {
            GameDataNEW.playerTournamentData.score += totalScore;
           // TournamentData tournamentData = new TournamentData(AccessToken.CurrentAccessToken.UserId, GameDataNEW.playerTournamentData.score, ProfileManager.UserProfile.gunNormalId, false);
           // FireBaseDatabase.Instance.SaveTournamentData(tournamentData);
        }
