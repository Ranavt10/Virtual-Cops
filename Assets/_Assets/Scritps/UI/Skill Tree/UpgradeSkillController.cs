...
public class UpgradeSkillController : MonoBehaviour
{
   ...
   // public TMP_Text textSkillName;
    ...
    {
        if (GameDataNEW.playerResources.gem < priceUpgrade)
        {
        ...
        {
            GameDataNEW.playerResources.ConsumeGem(priceUpgrade);
            UpgradeSkillSuccess();
        }
    }
    ...
    public void ResetPoints()
    {
        ...
        if (GameDataNEW.playerResources.gem < GetCostResetSkill())
        {
            Popup.Instance.ShowToastMessage("Not enough gems");
            return;
        }

        int usedPoints = GameDataNEW.playerRamboSkills.GetUsedSkillPoints(ramboId);
    ...
    {
        GameDataNEW.playerResources.ConsumeGem(GetCostResetSkill());
        GameDataNEW.playerRamboSkills[ramboId].Reset();
        CalculatePoints();
        LoadSkillTree();
        ResetUI();
    }
    ...
    {
        GameDataNEW.playerRamboSkills[ramboId].IncreaseLevel(selectingSkillId);
        LoadSkillTree();
        UpdateInformation();
        ...
        if (GameDataNEW.isShowingTutorial)
        {
            ...
            if (selectingSkillId == 6 && GameDataNEW.playerRamboSkills[ramboId].GetSkillLevel(selectingSkillId) == 1)
            {
                EventDispatcher.Instance.PostEvent(EventID.SubStepUnlockSkillPhoenixDown);
            }
        }

        StaticRamboSkillData skillData = GameDataNEW.staticRamboSkillData.GetData(selectingSkillId);
        if (skillData != null)
        {
           // FirebaseAnalyticsHelper.LogEvent("N_UpgradeSkill", skillData.skillName);

            if (skillData.type == SkillType.Active && GameDataNEW.playerRamboSkills.GetRamboSkillProgress(ramboId).GetSkillLevel(selectingSkillId) == 1)
            {
    ...
    {
        int ramboLevel = GameDataNEW.playerRambos.GetRamboLevel(ramboId);
        int usedSkillPoints = GameDataNEW.playerRamboSkills.GetUsedSkillPoints(ramboId);
    ...
    {
        PlayerRamboSkillData progress = GameDataNEW.playerRamboSkills.GetRamboSkillProgress(ramboId);
    ...
    private void LoadSkillOffense(PlayerRamboSkillData progress)
    {
        ...
        for (int i = 0; i < GameDataNEW.staticRamboSkillData.Count; i++)
        {
            StaticRamboSkillData data = GameDataNEW.staticRamboSkillData[i];
    ...
    private void UpdateResetSkillCost()
    {
        ...
        textPriceResetSkill.color = GameDataNEW.playerResources.gem >= cost ? Color.white : StaticValue.colorNotEnoughMoney;
    }

    private void UpdateInformation()
    {
        StaticRamboSkillData staticData = GameDataNEW.staticRamboSkillData.GetData(selectingSkillId);
           ...
           // textSkillName.text = staticData.skillName.ToUpper();
            ...
            if (level > 0)
            {
                ...
                if (!isMaxLevel)
                {
                    ...
                    textPriceUpgrade.color = GameDataNEW.playerResources.gem >= priceUpgrade ? Color.white : StaticValue.colorNotEnoughMoney;
                }
            }
            else
            {
    ...
    {
        int pointUsed = GameDataNEW.playerRamboSkills.GetUsedSkillPoints(ramboId);
