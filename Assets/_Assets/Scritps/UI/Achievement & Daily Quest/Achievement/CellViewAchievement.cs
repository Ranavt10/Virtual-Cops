...
public class CellViewAchievement : EnhancedScrollerCellView
{
    ...
    {
        StaticAchievementData staticData = GameDataNEW.staticAchievementData.GetData(_data.type);

        int curMilestoneIndex = GameDataNEW.playerAchievements.ContainsKey(_data.type) ?
                GameDataNEW.playerAchievements[_data.type].claimTimes : 0;
        ...
        else
        {
            ...
            _data.progress = GameDataNEW.playerAchievements.ContainsKey(staticData.type) ?
        ...
        if (GameDataNEW.playerAchievements.ContainsKey(_data.type))
        {
            GameDataNEW.playerAchievements[_data.type].claimTimes++;
