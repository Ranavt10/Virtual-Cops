...
public class HudAchievement : MonoBehaviour, IEnhancedScrollerDelegate
{
    ...
    {
        GameDataNEW.staticAchievementData.SortByState();
    ...
    {
        int count = GameDataNEW.playerAchievements.GetNumberReadyAchievement();
    ...
    private void CreateAchievementData()
    {
        ...
        for (int i = 0; i < GameDataNEW.staticAchievementData.Count; i++)
        {
            ...
            if (GameDataNEW.playerAchievements.ContainsKey(staticData.type))
            {
                curMilestoneIndex = Mathf.Clamp(GameDataNEW.playerAchievements[staticData.type].claimTimes, 0, staticData.milestones.Count - 1);
            }
            ...
            cell.progress = GameDataNEW.playerAchievements.ContainsKey(staticData.type) ?
                GameDataNEW.playerAchievements[staticData.type].progress : 0;

            cell.target = milestone.requirement;
            cell.rewards = milestone.rewards;
            cell.isCompleted = staticData.isCompleted;

            //cell.isCompleted = GameDataNEW.playerAchievements.ContainsKey(staticData.type) ?
            //    GameDataNEW.playerAchievements[staticData.type].claimTimes >= staticData.milestones.Count : false;
