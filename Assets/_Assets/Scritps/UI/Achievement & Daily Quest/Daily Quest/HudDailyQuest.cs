...
public class HudDailyQuest : MonoBehaviour, IEnhancedScrollerDelegate
{
    ...
    {
        int count = GameDataNEW.playerDailyQuests.GetNumberReadyQuest();
    ...
    private void CreateDailyQuestData()
    {
        ...
        {
            PlayerDailyQuestData playerQuest = GameDataNEW.playerDailyQuests[i];
            StaticDailyQuestData staticQuest = GameDataNEW.staticDailyQuestData.GetData(playerQuest.type);
    ...
    private void OnClaimReward(CellViewDailyQuestData data)
    {
        ...
        {
            int completedQuests = GameDataNEW.playerDailyQuests.Count(x => x.isClaimed && x.type != DailyQuestType.COMPLETE_ALL_QUEST);

            if (completedQuests == GameDataNEW.playerDailyQuests.Count - 1)
            {
