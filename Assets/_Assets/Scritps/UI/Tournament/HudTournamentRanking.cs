...
public class HudTournamentRanking : MonoBehaviour, IEnhancedScrollerDelegate
{
    ...
    private void Awake()
    {
        ...
        textPlayerFbName.text = GameDataNEW.playerTournamentData.fbName;
        imgPlayerAvatar.sprite = GameDataNEW.playerTournamentData.sprAvatar != null ? GameDataNEW.playerTournamentData.sprAvatar : sprLoadingAvatar;
    }
    ...
    private void CheckWeek()
    {
        ...
        if (GameDataNEW.playerTournamentData.isReceivedTopRankReward == false)
        {
            GameDataNEW.playerTournamentData.isReceivedTopRankReward = true;
           ...
           // FireBaseDatabase.Instance.GetTopTournamentForRewarded(data =>
            {
               ...
               // for (int i = data.Count - 1; i >= 0; i--)
                ...
                {
                    if (GameDataNEW.tournamentTopRankRewards.ContainsKey(rank - 1))
                    {
                        List<RewardData> rewards = GameDataNEW.tournamentTopRankRewards[rank - 1];
    ...
    private void CheckNotification()
    {
        ...
        if (ProfileManager.UserProfile.countPlayTournament < StaticValue.TOURNAMENT_MAX_ENTRANCE
            && GameDataNEW.playerResources.tournamentTicket > 0)
        {
            textFreeEntrance.text = GameDataNEW.playerResources.tournamentTicket.ToString();
   ...
   //         TournamentRank curRank = GameDataNEW.staticTournamentRankData.GetCurrentRank(data.score);
   //         StaticTournamentRankData staticData = GameDataNEW.staticTournamentRankData.GetData((int)curRank);
   ...
   //         if (rankIndex < 3)
   //         {
   //             if (GameDataNEW.tournamentTopRankRewards.ContainsKey(rankIndex))
   //                 cellViewData.rewards = GameDataNEW.tournamentTopRankRewards[rankIndex];
   //         }
   ...
   // }
    ...
    public void Challenge()
    {
        ...
        {
            if (GameDataNEW.playerResources.gem < priceChallenge)
            {
    ...
    {
        TournamentRank playerCurRank = GameDataNEW.staticTournamentRankData.GetCurrentRank(GameDataNEW.playerTournamentData.score);

        textPlayerScore.text = GameDataNEW.playerTournamentData.score.ToString();
        ...
        if (playerCurRank >= TournamentRank.Legend)
        {
            ...
            textRankProgress.text = GameDataNEW.playerTournamentData.score.ToString();
        }
        else
        {
            int nextRankIndex = (int)playerCurRank + 1;
            StaticTournamentRankData curRankData = GameDataNEW.staticTournamentRankData.GetData((int)playerCurRank);
            StaticTournamentRankData nextRankData = GameDataNEW.staticTournamentRankData.GetData(nextRankIndex);

            float percent = Mathf.Clamp01((float)(GameDataNEW.playerTournamentData.score - curRankData.score) / (float)(nextRankData.score - curRankData.score));
            imgRankProgress.fillAmount = percent;
            textRankProgress.text = string.Format("{0}/{1}", GameDataNEW.playerTournamentData.score, nextRankData.score);
        }

        //remainingTickets.text = ProfileManager.UserProfile.countFreeTournamentTicket.ToString();
        //medal.text = GameDataNEW.playerResources.medal.ToString();
        ...
        {
            if (GameDataNEW.playerResources.tournamentTicket > 0)
            {
                textFreeEntrance.transform.parent.gameObject.SetActive(true);
                textFreeEntrance.text = GameDataNEW.playerResources.tournamentTicket.ToString();
                groupGem.SetActive(false);
            ...
            else
            {
                ...
                if (ProfileManager.UserProfile.countPlayTournament >= StaticValue.TOURNAMENT_FREE_ENTRANCE)
                {
                    ...
                    textGemChallenge.color = GameDataNEW.playerResources.gem >= priceChallenge ? Color.white : StaticValue.colorNotEnoughMoney;
                    textGemChallenge.text = priceChallenge.ToString();
                }
                else
                {
                    GameDataNEW.playerResources.ReceiveTournamentTicket(StaticValue.TOURNAMENT_FREE_ENTRANCE);
                    groupGem.SetActive(false);
                    groupTicket.SetActive(true);
                    priceChallenge = 0;
                    FillInformation();
