...
public class Rambo : BaseUnit
{
    ...
    {
        level = GameDataNEW.playerRambos.GetRamboLevel(ProfileManager.UserProfile.ramboId);
        grenadeLevel = GameDataNEW.playerGrenades.GetGrenadeLevel(ProfileManager.UserProfile.grenadeId);
        InitSkills();
    ...
    protected virtual void CalculateBaseStatsIncrease()
    {
        ...
        {
            GameDataNEW.isAutoCollectCoin = false;
            ...
            for (int i = 0; i < GameDataNEW.selectingBoosters.Count; i++)
            {
                ...
                switch (type)
                {
                    case BoosterType.CoinMagnet:
                        GameDataNEW.isAutoCollectCoin = true;
                        break;
        ...
        if (stats.HP <= 0)
        {
            ...
            if (GameDataNEW.mode == GameMode.Campaign)
            {
                if (((BaseEnemy)attackData.attacker).isFinalBoss)
                {
                    ...
                    //    string.Format("ID={0},{1}-{2}", ((BaseEnemy)attackData.attacker).id, GameDataNEW.currentStage.id, GameDataNEW.currentStage.difficulty));
                }
            }
        }
    }
    ...
    private void InitWeapons()
    {
        ...
        if (normalGunPrefab != null)
        {
            ...
            int level = GameDataNEW.playerGuns.GetGunLevel(ProfileManager.UserProfile.gunNormalId);
            //int level = GameDataNEW.playerGuns.GetGunLevel(8);
        ...
        if (specialGunPrefab != null)
        {
            ...
            int level = GameDataNEW.playerGuns.GetGunLevel(ProfileManager.UserProfile.gunSpecialId);
            //int level = GameDataNEW.playerGuns.GetGunLevel(108);
        ...
        if (meleeWeaponPrefab != null)
        {
            ...
            int level = GameDataNEW.playerMeleeWeapons.GetMeleeWeaponLevel(ProfileManager.UserProfile.meleeWeaponId);
            //int level = GameDataNEW.playerMeleeWeapons.GetMeleeWeaponLevel(601);
        ...
        numberOfGrenade = GameDataNEW.playerGrenades.ContainsKey(grenadeId) ? GameDataNEW.playerGrenades[grenadeId].quantity : 0;
    ...
    protected virtual void ReleaseGrenade()
    {
        ...
        UIController.Instance.UpdateGrenadeText(numberOfGrenade);
        GameDataNEW.playerGrenades.Consume(ProfileManager.UserProfile.grenadeId, 1);
    }
    ...
    private void GetGunDrop(int id)
    {
        if (GameDataNEW.staticGunData.ContainsKey(id))
        {
            if (dropGun == null || dropGun.id != id)
            {
                ...
                SO_GunStats staticGun = GameDataNEW.staticGunData.GetBaseStats(id, 1);
    ...
    private void OnToggleAutoFire()
    {
        ...
        GameDataNEW.isAutoFire = isAutoFire;
    }
    ...
    private void OnOutGamePlay()
    {
        if (specialGun != null)
        {
            GameDataNEW.playerGuns.SetGunAmmo(ProfileManager.UserProfile.gunSpecialId, specialGun.ammo);
        }
    }
    ...
    protected virtual void OnCompleteSurvivalWave()
    {
        ...
        GameDataNEW.survivalUsingBooster = BoosterType.None;
    ...
    protected virtual void OnUseSupportBooster(BoosterType type)
    {
        ...
        GameDataNEW.survivalUsingBooster = type;
        UIController.Instance.ActiveBoosters();
        ...
        if (GameDataNEW.mode == GameMode.Survival)
        {
            if (GameDataNEW.survivalUsingBooster == BoosterType.Damage)
            {
                isDamageBuffed = true;
                AddModifier(new ModifierData(StatsType.Damage, ModifierType.AddPercentBase, 0.15f));
            }
            else if (GameDataNEW.survivalUsingBooster == BoosterType.Critical)
            {
                isCriticalBuffed = true;
                AddModifier(new ModifierData(StatsType.CriticalRate, ModifierType.AddPercentBase, 0.1f));
            }
            else if (GameDataNEW.survivalUsingBooster == BoosterType.Speed)
            {
                isSpeedBuffed = true;
