...
public class EventDispatcher : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (s_instance != null && s_instance.GetInstanceID() != this.GetInstanceID())
        {
            ...
            Common.Log("An instance of EventDispatcher already exist : <{0}>, So destroy this instance : <{1}>!!", s_instance.name, name);
            Destroy(gameObject);
        }
        else
        {
    ...
    void OnDestroy()
    {
        ...
        Common.Log("Destroy EventDispatcher");
    }
        ...
        if (_listenersDict.TryGetValue(eventID, out actionList))
        {
            ...
            for (int i = 0, amount = actionList.Count; i < amount; i++)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Debug.LogError(string.Format("Remove listener at {0} that cause the exception, message: {1}", eventID.ToString(), e.Message));
