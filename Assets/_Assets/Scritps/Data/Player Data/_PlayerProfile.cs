...
public class _PlayerProfile
{
    ...
    public void ReceiveExp(int value)
    {
        if (value > 0)
        {
            ...
            int curLevel = GameDataNEW.staticRankData.GetLevelByExp(exp);
            ...
            if (curLevel > level)
            {
                ...
                StaticRankData newRankData = GameDataNEW.staticRankData.GetData(level);
                string msg = string.Format("RANK UP TO LEVEL {0}\n<color=yellow>{1}</color>",
                    newRankData.level, GameDataNEW.staticRankData.GetRankName(newRankData.level));
