...
public class _PlayerMeleeWeaponData : Dictionary<int, PlayerMeleeWeaponData>
{
    ...
    {
        if (GameDataNEW.staticMeleeWeaponData.ContainsKey(id))
        {
            if (ContainsKey(id))
            {
                ...
                weapon.level += 2;
                weapon.level = Mathf.Clamp(weapon.level, 1, GameDataNEW.staticMeleeWeaponData[id].upgradeInfo.Length);
            }
            else
            {
                StaticMeleeWeaponData staticData = GameDataNEW.staticMeleeWeaponData.GetData(id);
