    ...
    public void Spawn(List<ItemDropData> items, Vector2 position, BaseEnemy unitDrop = null)
    {
        if (items.Count > 0)
        {
            for (int i = 0; i < items.Count; i++)
            {
                ...
                {
                    bool isSurvivalMode = GameDataNEW.mode == GameMode.Survival;
                    bool isCampaignNormal = GameDataNEW.mode == GameMode.Campaign && GameDataNEW.currentStage.difficulty == Difficulty.Normal;
