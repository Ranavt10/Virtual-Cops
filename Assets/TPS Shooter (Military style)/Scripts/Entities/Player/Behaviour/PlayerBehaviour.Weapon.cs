...
namespace TPSShooter
{
    public partial class PlayerBehaviour
    {
        ...
        private void InitializeWeapon()
        {
            ...
            {
                photonView.RPC(nameof(SetCurrentWeapon), RpcTarget.AllBuffered, weaponIndex);//SetCurrentWeapon(weaponIndex);
            }
        }
        ...
        private void Fire()
        {
            ...
            photonView.RPC(nameof(setIcon), RpcTarget.Others);

            //minimapCoroutine = StartCoroutine(setMiniMapHit());
            /*if (photonView.IsMine)
                minimapCamera.FireWeapon();*/
        ...
        public void setIcon()
        {
            enemyMiniMapComponent.enabled = true;
            enemyDetection.SetActive(true);

            CancelInvoke(nameof(InvokeOffIcon));
            Invoke(nameof(InvokeOffIcon), 3f);
        }

        private void InvokeOffIcon()
        {
            enemyMiniMapComponent.enabled = false;
            enemyDetection.SetActive(false);
        }

        /*private IEnumerator setMiniMapHit()
        {
            yield return null;
            if (photonView.IsMine)
            {
        ...
        /*private IEnumerator checkFireStoppage()
        {
            while (true)
            {
