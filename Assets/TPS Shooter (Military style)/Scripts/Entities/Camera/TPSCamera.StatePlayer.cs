    ...
    public partial class TPSCamera
    {
        private class CameraPlayerState : CameraState
        {
            ...
            public override void OnEnter()
            {
                ...
                tpsCamera.cameraContainer.localPosition = new Vector3(tpsCamera.containerCameraVal.x, tpsCamera.containerCameraVal.y,
                    tpsCamera.containerCameraVal.z);
            }
            ...
            {
                RaycastHit hit;
                Vector3 pivotPos = tpsCamera.collidedPosition/*pivot*/.position;
                Vector3 dir = tpsCamera.cameraContainer.position - pivotPos;
                float dist = Mathf.Abs(cachedContainerLocalPos.z);

                if (Physics.SphereCast(pivotPos, 0.1f, dir, out hit, 1.3f/*dist*/, tpsCamera.playerCameraSettings.wallLayers))
                {
