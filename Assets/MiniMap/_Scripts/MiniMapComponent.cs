...
public class MiniMapComponent : MonoBehaviour {
	...
	void OnEnable(){
		miniMapController = MiniMapManager.Instance.mapController;//GameObject.Find ("CanvasMiniMap").GetComponent<MiniMapController> ();
	...
	void OnDestroy(){
		if (miniMapController != null && mmo != null && this.gameObject != null)
			miniMapController.UnregisterMapObject (mmo,this.gameObject);
	}
}
